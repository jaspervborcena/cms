...
- Multi-store support per account with store selection in the sidebar
- Onboarding flow for first store creation and setup
- `CmsService` using Angular signals for posts, stores, and active store state
...
- Left admin navigation with store list and post actions
...
- Published public site routes under `/site/:storeId` and `/site/:storeId/:slug`
- Page-based public site navigation and CMS-managed page rendering
- Theme selection with default theme fallback for published store pages
...
- **Save Draft**
  - `/posts/new` should save drafts with `status: "draft"` under `/stores/{storeId}/posts/{postId}`.
  - Drafts remain private and editable.
- **Preview**
  - Preview should load at `/preview/:storeId/:postId`.
  - This route renders the post inside the store theme layout with header, footer, sidebar, and widgets.
  - A preview should use the selected store theme so the editor sees the real site styling.
- **Publish**
  - Published content should appear on the public site route `/site/:storeId`.
...
- Each store has a `themeId` in Firestore.
- Preview route should load the theme HTML/CSS and inject the post content.
- Public site uses the same theme consistently for `/site/:storeId`.

### Editor Action Buttons

- **Save Draft** → create or update a post with `status: "draft"`.
- **Preview** → navigate to `/preview/:storeId/:postId`.
- **Publish** → set `status: "published"` and redirect to `/site/:storeId`.

## Key Files

- `src/app/services/cms.service.ts` — CMS state, post/store CRUD, active store management
...
- `src/app/guards/ensure-store.guard.ts` — Guard to ensure an active store is selected before entering protected routes
...
- Filter dashboard content by the selected active store
- Add `/dashboard/:storeId` deep links and store switching in the UI
- Harden Firestore security rules for per-user store ownership
...
- Uses hostname parsing so `www.slug.cms.tovrika.com/test` can resolve the correct store and post.
...
- Harden Firestore security rules for per-user store ownership
- Improve editor styling to more closely match the admin screenshot
- Add better author and store settings pages
